...
# --- Initialize Dash App ---
# Switched to a modern, light theme: PULSE
...
login_layout = dbc.Container([
    dbc.Row(dbc.Col(html.H1("GCash Internal Database Access", className="display-4 fw-bold"), width=12), className="my-5 text-center"),
    dbc.Row(
        dbc.Col(
            dbc.Card([
                dbc.CardHeader(html.H4("Login", className="mb-0")), # Added className mb-0
                dbc.CardBody([
                    dbc.Form([
                        html.Div([
...
], fluid=True, className="pt-5") # Added pt-5 for some top padding

# Navbar component
navbar = dbc.NavbarSimple(
    children=[], # Will be updated by callback
    brand="GCash DB Access System",
    brand_href="/",
    color="primary",
    dark=True,
    sticky="top",
    id="app-navbar",
    className="mb-4" # Add margin bottom to navbar
)

# Dashboard Layout
def create_dashboard_layout(session_data):
    if not session_data or not session_data.get('logged_in'):
        # This case should ideally be handled by the display_page callback redirecting to login
        app.logger.warning("create_dashboard_layout called without valid session. Redirecting logic should prevent this.")
        return login_layout # Fallback, though redirect is preferred
    ...
    new_request_button_ui = dbc.Row(
        dbc.Col(
            dbc.Button("New Access Request", id="open-new-request-modal-button", color="success", className="mb-3 mt-3", size="lg"),
            width={"size": "auto"} # Auto width for the button
        ),
        justify="start", # Align button to the left
        className="mb-4"
    )
    ...
    my_requests_section_ui = dbc.Card([
        dbc.CardHeader(html.H4("My Access Requests", className="mb-0")),
        dbc.CardBody([
            dash_table.DataTable(
                ...
                style_header={'fontWeight': 'bold', 'backgroundColor': '#f8f9fa'}, # Light header for PULSE
                style_table={'overflowX': 'auto'},
                style_data_conditional=[{'if': {'row_index': 'odd'}, 'backgroundColor': '#fdfdfe'}], # Very subtle odd row coloring
                page_size=10,
                row_selectable='single',
                selected_rows=[],
            ),
            html.Div(id='my-request-action-panel', className="mt-3 p-3", style={'display': 'none'}) # Removed border, let card handle
        ])
    ], className="mb-4")
    ...
    if is_manager:
        ...
        approval_section_content = [
            dbc.Card([
                dbc.CardHeader(html.H4("Requests Requiring My Action / History", className="mb-0")),
                dbc.CardBody([
                    dash_table.DataTable(
                        ...
                        style_table={'overflowX': 'auto'},
                        style_data_conditional=[{'if': {'row_index': 'odd'}, 'backgroundColor': '#fdfdfe'}],
                        page_size=5,
                        row_selectable='single',
                        selected_rows=[],
                    ),
                    html.Div(id='approval-action-panel', className="mt-3 p-3", style={'display': 'none'}) # Removed border
                ])
            ], className="mb-4")
        ]
    else:
        ...
        approval_section_content = [
            html.Div(id='approval-requests-table', style={'display': 'none'}), # Keep IDs for callbacks
            html.Div(id='approval-action-panel', style={'display': 'none'})
            ]

    reports_section_ui = dbc.Card([
        dbc.CardHeader(html.H4("Generate Reports", className="mb-0")),
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    dcc.Dropdown(
                ...
                ], md=6), # Use md for medium devices
                dbc.Col([
    ...
    return dbc.Container([
        ...
        dbc.Row([
            # Welcome message removed from here as it's on the navbar, or can be simpler
            dbc.Col(html.H2("Dashboard Overview"), width=12),
        ], className="mb-4 mt-2 align-items-center"), # Reduced mt-4 to mt-2
        html.Div(id='action-feedback-alert-placeholder', className="mb-3"), # Added mb-3 for spacing
...
app.layout = html.Div([
    ...
    html.Div(id='page-content', className="mt-4") # Added mt-4 for spacing below sticky navbar
])
...
def update_navbar(session_data):
    ...
    app.logger.info(f"update_navbar: is_logged_in={is_logged_in}")
    if is_logged_in:
        user_first_name = session_data.get('first_name', 'User')
        # Using NavLink for href behavior, n_clicks will be None initially for NavLink
        return [
            dbc.NavItem(dbc.NavLink(f"Welcome, {user_first_name}!", disabled=True, style={"color": "rgba(255,255,255,.75)"})), # Lighter color for disabled
            dbc.NavItem(dbc.NavLink("Logout", id="logout-button-nav", href="/login", n_clicks=0))
        ]
    return []
...
def display_page(pathname, session_data):
    ...
    elif pathname == '/dashboard' and not is_logged_in: # Trying to access dashboard but not logged in
        app.logger.info("display_page: Access to /dashboard denied, not logged in. Redirecting to /login.")
        return login_layout # Or implement a redirect to /login
    elif pathname == '/' and is_logged_in: # If at root and logged in, go to dashboard
        app.logger.info("display_page: At root and logged in, redirecting to /dashboard.")
        # This needs a dcc.Location redirect in practice, or ensure dashboard layout is returned.
        # For simplicity here, we'll let the navbar handle dashboard access,
        # and assume direct /dashboard navigation by user or after login.
        # A more robust way is to redirect using dcc.Location from a callback.
        # For now, this flow relies on the login success redirecting to /dashboard.
        return create_dashboard_layout(session_data)
    else: # Default to login page (covers /login, /, or any other path when not logged in)
        app.logger.info(f"display_page: Defaulting to login_layout for pathname: {pathname}")
        return login_layout
...
@app.callback(
    ...
    prevent_initial_call=True # Essential for n_clicks with NavLink
)
def handle_logout_nav(n_clicks_logout):
    app.logger.info(f"handle_logout_nav: n_clicks_logout={n_clicks_logout} (type: {type(n_clicks_logout)})")
    # n_clicks for NavLink is 0 initially, then 1 on first click.
    # It does not increment beyond 1 unless page is reloaded or component re-rendered.
    if n_clicks_logout and n_clicks_logout > 0:
        app.logger.info("User logged out via navbar.")
        return {}, '/login' # Clear session, redirect to login
    app.logger.info("handle_logout_nav: No effective logout click.")
...
# --- Existing callbacks for table updates, actions, modal, reports ---
# (No changes to the logic of these callbacks, only to the layout they interact with)
...
def update_approval_requests_table(n_intervals_load, refresh_trigger, session_data, pathname):
    ...
    session_data = session_data or {}
    # Ensure the approval section is only processed if the elements exist (i.e., user is manager)
    if not (session_data.get('logged_in') and session_data.get('is_manager') and pathname == '/dashboard'):
        app.logger.info(f"update_approval_requests_table: Conditions not met (not logged in, not manager, or not on dashboard).")
        return [], [], {'display': 'none'}, [] # Hide table if not applicable
    ...
    conn = get_db_connection()
    if not conn: return [], [], {'overflowX': 'auto', 'display': 'block'}, [] # Show table structure on error
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT ar.request_id, req_emp.first_name || ' ' || req_emp.last_name AS requester_name, req_emp.email AS requester_email, dt.schema_name || '.' || dt.table_name AS table_full_name, aro.role_name AS requested_role, ar.justification, ar.request_date, ar.status FROM AccessRequests ar JOIN Employees req_emp ON ar.requester_id = req_emp.employee_id JOIN DatabaseTables dt ON ar.table_id = dt.table_id JOIN AccessRoles aro ON ar.requested_role_id = aro.role_id WHERE req_emp.manager_id = %s ORDER BY CASE ar.status WHEN 'Pending' THEN 0 ELSE 1 END, ar.request_date DESC;", (manager_id,))
...
def update_my_request_action_panel(selected_rows, table_data, session_data):
    ...
    if request_status == 'Pending' and requester_id_from_table == current_user_id:
        ...
        return panel_content, {'display': 'block', 'marginTop': '15px', 'padding': '15px'}, request_id # Simplified style
...
def update_approval_action_panel(selected_rows, table_data):
    ...
    if request_status == 'Pending':
        panel_content = [
            html.H5(f"Action for Request ID: {request_id}", className="mb-3"), html.P(f"Requester: {selected_request['requester_name']} ({selected_request['requester_email']})"),
        ...
        return panel_content, {'display': 'block', 'marginTop': '15px', 'padding': '15px'}, request_id # Simplified style
    else: # Display details for already actioned items
        panel_content = [
        ...
        return panel_content, {'display': 'block', 'marginTop': '15px', 'padding': '15px'}, request_id # Simplified style
    # app.logger.info(f"Approval Action Panel: Request {request_id} not pending."); return [], {'display': 'none'}, None # This line is unreachable due to else block
...
def handle_approval_decision(approve_clicks, reject_clicks, request_id, comment_text, session_data, current_refresh_count):
    ...
    # Check which button was clicked by examining the triggered_prop_ids
    ...
    try:
        with conn.cursor() as cur:
            # Ensure the approver is actually the manager of the requester
            cur.execute("""
...
def submit_new_request(n_clicks_submit, table_id, role_id, justification, session_data, current_refresh_count):
    ...
    if not session_data or not session_data.get('logged_in'):
        ...
        # modal_is_open = False # Keep modal open so user sees error
        return modal_feedback, new_refresh_count, modal_is_open, reset_table, reset_role, reset_justification, global_feedback
    ...
    try:
        with conn.cursor() as cur:
            ...
            modal_is_open = False # Close modal on success
            modal_feedback = "" # Clear modal feedback
            reset_table, reset_role, reset_justification = None, None, "" # Reset form fields
    except psycopg2.Error as e:
        conn.rollback()
        ...
        for col in df.columns:
            if 'date' in col.lower() or 'timestamp' in col.lower():
                try:
                    ...
                    elif pd.api.types.is_object_dtype(df[col]): # Could be strings if already formatted or mixed
                         # Attempt conversion if they look like datetimes but are objects

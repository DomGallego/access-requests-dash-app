...
def create_dashboard_layout(session_data):
    ...
    return dbc.Container([
        ...
        *new_request_button_ui, # New request button
        new_request_modal,      # New request modal
        *my_requests_section_ui,
        *approval_section_content
    ], fluid=True)
...
def toggle_and_populate_new_request_modal(n_open, n_cancel, is_open_state, session_data):
    ...
    # Always reset form fields when opening or cancelling
    reset_table_val, reset_role_val, reset_just_val = None, None, ""
    modal_specific_feedback = "" # Feedback for inside the modal
    ...
    if triggered_id == 'open-new-request-modal-button' and n_open:
        ...
        if conn and session_data and session_data.get('logged_in'):
            ...
            # Not opening modal if critical data can't be loaded or not logged in
    ...
    if triggered_id == 'cancel-new-request-modal-button' and n_cancel:
        ...
        return False, dash.no_update, dash.no_update, reset_table_val, reset_role_val, reset_just_val, "" # Clear feedback on cancel

    # Default case if no specific trigger condition is met (e.g., initial load, or submit button handled by another callback)
...
@app.callback(
    [Output('new-request-form-feedback', 'children', allow_duplicate=True), # Modal-specific feedback
     Output('refresh-trigger-store', 'data', allow_duplicate=True),
     Output('new-request-modal', 'is_open', allow_duplicate=True), # Control modal open/close
     # Reset form fields on success by outputting their new (empty) values
     Output('new-request-table-dropdown', 'value', allow_duplicate=True),
     Output('new-request-role-dropdown', 'value', allow_duplicate=True),
     Output('new-request-justification-textarea', 'value', allow_duplicate=True),
     Output('action-feedback-alert-placeholder', 'children', allow_duplicate=True)], # Global feedback for success
...
def submit_new_request(n_clicks_submit, table_id, role_id, justification, session_data, current_refresh_count):
    ...
    modal_feedback, new_refresh_count, modal_is_open = no_update, no_update, True # Keep modal open by default on error
    global_feedback = no_update
    reset_table, reset_role, reset_justification = no_update, no_update, no_update # Don't reset by default
    ...
    if not session_data or not session_data.get('logged_in'):
        ...
        modal_is_open = False # Close modal if fundamental auth error
        return modal_feedback, new_refresh_count, modal_is_open, reset_table, reset_role, reset_justification, global_feedback
    ...
    try:
        with conn.cursor() as cur:
            ...
            modal_feedback = "" # Clear modal feedback on success
            reset_table, reset_role, reset_justification = None, None, "" # Reset form fields for next time
    except psycopg2.Error as e:
        conn.rollback()

...
from dash_iconify import DashIconify # For icons
...
# Login Layout - Reverted to explicit column widths within a standard container
login_layout = dbc.Container([ # REMOVED fluid=True, added className for potential custom styling
    dbc.Row(
    ...
    dbc.Row(
        ...
        # justify="center", # Center the row content if it doesn't fill the container
        align="start", # Keep columns top-aligned
        className="mt-3 gx-lg-4" # Gutter between columns on large screens
    )
# Standard container will center itself and have a max-width.
# px-0 will remove default horizontal padding from container if you want cols to go edge-to-edge of this max-width.
# Or keep default padding by removing px-0.
], id="login-container-main", className="py-5") # Changed ID for clarity, no fluid=True
...
def create_main_content_area(session_data):
    ...
    approval_section_ui = dbc.Card([
        ...
        dbc.CardBody([
            # DataTable for approvals - its data and style are handled by a callback
            dash_table.DataTable(
                id='approval-requests-table',
                # Minimal styles here, callback will update them
                style_cell={'textAlign': 'left', 'padding': '10px', 'whiteSpace': 'normal', 'height': 'auto', 'minWidth': '120px'},
                style_header={'fontWeight': '600', 'backgroundColor': '#e9ecef'},
                style_table={'overflowX': 'auto'}, # Default style
                page_size=5, row_selectable='single', selected_rows=[],
            ),
            # Action panel for approvals - its children and style are handled by a callback
...
app.layout = html.Div([
    ...
    html.Div(id='app-container-wrapper')
])
...
def render_page_content(pathname, session_data):
    ...
    if is_logged_in:
        ...
        return html.Div([
            ...
            create_main_content_area(session_data)
        ], id="app-container")
    else:
        return login_layout
...
@app.callback(
    ...
    [State('session-store', 'data')], # Removed pathname state as it's less reliable here
    prevent_initial_call=True
)
...
@app.callback(
    [Output('approval-requests-table', 'data'), Output('approval-requests-table', 'columns'),
     ...
     Output('approval-section-card', 'style')], # Added to control card visibility
    [Input('dashboard-load-trigger', 'n_intervals'), Input('refresh-trigger-store', 'data')],
    [State('session-store', 'data')], # Removed pathname state
    prevent_initial_call=True
)
def update_approval_requests_table(n_intervals_load, refresh_trigger, session_data):
    ...
    session_data = session_data or {}
    is_manager = session_data.get('is_manager', False)
    card_style = {'display': 'block' if is_manager else 'none'} # Card visibility
    ...
    if not (session_data.get('logged_in') and is_manager):
        ...
        # Return empty data for table and hide the card
    ...
    conn = get_db_connection()
    table_style_visible = {'overflowX': 'auto', 'display': 'block'} # Style when table should be visible

    if not conn:
        return [], [], table_style_visible, [], card_style # Show card, but table might be empty due to DB error
...
@app.callback(
    ...
    [State('approval-requests-table', 'data'), State('session-store', 'data')] # Added session-store
)
...
def handle_approval_decision(approve_clicks, reject_clicks, request_id, comment_text, session_data, current_refresh_count):
    ...
    session_data = session_data or {}
    if not session_data.get('is_manager'): # Non-managers should not trigger this
...
# --- Callbacks for New Request Modal (Triggered from Sidebar Button) ---
@app.callback(
